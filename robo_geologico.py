...
# 1. Configuração da IA (Secrets do GitHub)
...
# 2. Gerenciamento do Banco de Dados (CSV)
...
# 3. Varredura Total para Série Histórica
print("🔎 Buscando matérias novas e passadas...")
novas_encontradas = []
for fonte in fontes:
    try:
...
# 4. Processamento de Lote (30 notícias para limpar o atraso)
pendentes = pd.concat([pd.DataFrame(novas_encontradas), hist[hist['resumo'] == 'Pendente']])
ja_prontas = hist[hist['resumo'] != 'Pendente']

# Focamos em 30 para não estourar o tempo do GitHub mas limpar a fila rápido
a_processar = pendentes.head(30)
restante_fila = pendentes.iloc[30:]

print(f"📈 {len(pendentes)} notícias na fila. Processando lote de 30 agora...")

processadas_agora = []
for i, n in a_processar.iterrows():
    try:
        ...
        if len(texto) > 300:
            ...
            print(f"✅ Processado: {n['titulo'][:40]}...")
            time.sleep(22) # Pausa estratégica segura
    except Exception as e:
        print(f"⏳ Erro ou Cota: {n['titulo'][:20]}. Voltando para a fila.")
        n['resumo'] = 'Pendente'
        processadas_agora.append(n)

# 5. União e Salvamento
df_final = pd.concat([pd.DataFrame(processadas_agora), restante_fila, ja_prontas]).drop_duplicates(subset='link')
df_final.to_csv(arquivo_hist, index=False, encoding='utf-8-sig')

# 6. Geração do HTML (Interface Completa)
...
<body>
    <div class="container">
        <h1>⛏️ Radar Mineral - Cecília Rabelo</h1>
        <div class="filters">
            <input type="text" id="busca" placeholder="🔍 Pesquisar em títulos, resumos ou #keywords..." onkeyup="filtrar()">
